...
/* 캐시 리스트에서 값을 가져와서 메모리에 매핑하고, 
가장 최근에 사용한 것이므로 캐시 리스트 맨 앞으로 이동함 (이 과정의 에러를 막기 위해 락-언락 사용)*/
int read_node_content(c_list *list, char *index, char *content, unsigned int *len){
	if(!list)
		return -1;
	
	pthread_rwlock_rdlock((list->lock));        // rdlock = 읽는 스레드가 하나라도 있을 때 , 언락 = 읽는 스레드가 없을 때
	...
	*len = tmp->length;
	memcpy(content, tmp->content,*len);         // search_node를 통해 찾은 tmp의 컨텐츠를 내 메모리에 카피
	
	pthread_rwlock_unlock((list->lock));
	
	pthread_rwlock_wrlock((list->lock));
	add_node(remove_node(index, list), list);   // 페이지 교체 알고리즘 고려
	pthread_rwlock_unlock((list->lock));
